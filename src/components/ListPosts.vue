...
<template>
  <ul sm:min-h-38 min-h-28>
    ...
    <li v-for="(posts, index) in list " :key="posts.data.title" mb-8>
      ...
      <a text-lg lh-tight nav-link flex="~ col gap-2" :aria-label="posts.data.title" :target="getTarget(posts)" :href="getHref(posts)">
        <div flex="~ col md:row gap-2 md:items-center">
          ...
          <div opacity-50 text-sm ws-nowrap flex="~ gap-2 items-center">
            ...
            <span v-if="posts.data.lang && posts.data.lang.includes('zh')">· 中文</span>
          </div>
        </div>
